"""
Agent 1: Explorer Agent Module

An exploration-focused agent that prioritizes discovering new locations and mapping the world.
Based on LocationSimpleAgent with emphasis on exploration and discovery.

Key features:
- Prioritizes exploring unmapped areas
- Records detailed location information
- Focus on discovering new routes and towns
"""
...
@dataclass
class ExplorerAgentState:
    """Maintains minimal state for the explorer agent"""
    history: deque = None
...
class ExplorerAgent:
    """
    Explorer agent that focuses on discovering and mapping new locations.
    """
    
    def __init__(self, vlm):
        self.vlm = vlm
        self.state = ExplorerAgentState()
        self.memory_manager = MemoryManager()
        
        self.memory_file = ".pokeagent_cache/memory_explorer.json"
        ...
        logger.info("Initialized ExplorerAgent with memory management")
    ...
    def get_location_memory_context(self, map_name: str) -> str:
        ...
        if current_memory:
            ...
            lines.append(f"📍 CURRENT LOCATION ({map_name}): ⭐ NEW UNEXPLORED AREA! Record everything you see!")
        
        all_memories = self.memory_manager.get_all_location_memories()
        other_memories = {k: v for k, v in all_memories.items() if k != map_name}
        
        if other_memories:
            lines.append(f"\n🗺️ EXPLORED LOCATIONS ({len(other_memories)} discovered):")
            ...
            if len(other_memories) > 3:
                lines.append(f"   ... and {len(other_memories) - 3} more locations discovered")
        
        return "\n".join(lines) if lines else "No locations explored yet - adventure awaits!"
    ...
    def step(self, game_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if frame is None:
            logger.error("🚫 No frame in game_state for ExplorerAgent.step")
            return {"action": "WAIT", "reasoning": "No frame available"}
        
        action = self.process_step(frame, game_state)
        return {"action": action, "reasoning": "Explorer agent decision"}
    
    def process_step(self, frame, game_state: Dict[str, Any]) -> str:
        """Main processing step for explorer mode."""
        if frame is None:
            logger.error("🚫 CRITICAL: ExplorerAgent.process_step called with None frame")
            return "WAIT"
        ...
        try:
            ...
            # Explorer-specific prompt
            prompt = f"""You are an EXPLORER playing Pokemon Emerald. Your mission: DISCOVER AND MAP new locations!

🎯 EXPLORER OBJECTIVES:
- Prioritize visiting unexplored areas (locations without memory)
- Record detailed observations about each location
- Note connections between areas, NPCs, items, and landmarks
- Seek out new routes, towns, and hidden areas
...
Instructions:
1. Look for unexplored exits and new areas
2. Document everything you discover about this location
3. Prioritize exploration over battles or training

Response format:
ANALYSIS:
[Is this a new location? What can you see? Any unexplored exits?]

LOCATION_MEMORY: [Record detailed info: NPCs, items, exits, landmarks, connections to other areas]

PLAN:
[What's your exploration strategy? Which direction leads to unexplored areas?]

ACTION:
[Your chosen action - prefer movement to explore]

Map: {map_name} | Explored: {len(self.memory_manager.get_all_location_memories())} locations | Step: {self.state.step_counter}"""
            
            print("\n" + "="*100)
            print("🗺️ EXPLORER AGENT PROMPT:")
            print("="*100)
            ...
            response = self.vlm.get_query(frame, prompt, "explorer_mode", game_state=game_state)
            ...
            logger.error(f"Error in explorer agent processing: {e}")
            return "A"
...
_global_explorer_agent = None


def get_explorer_agent(vlm):
    """Get or create the global explorer agent instance"""
    global _global_explorer_agent
    if _global_explorer_agent is None:
        _global_explorer_agent = ExplorerAgent(vlm)
    elif _global_explorer_agent.vlm != vlm:
        _global_explorer_agent = ExplorerAgent(vlm)
    
    return _global_explorer_agent
