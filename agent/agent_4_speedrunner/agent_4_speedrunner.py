"""
Agent 4: Speedrunner Agent Module

A speed-focused agent that prioritizes efficient progression through the game.
Based on LocationSimpleAgent with emphasis on quick completion and optimal routing.

Key features:
- Prioritizes main story progression
- Makes quick, efficient decisions
- Focus on optimal routing and minimal backtracking
"""
...
@dataclass
class SpeedrunnerAgentState:
    """Maintains minimal state for the speedrunner agent"""
    history: deque = None
...
class SpeedrunnerAgent:
    """
    Speedrunner agent that focuses on efficient game progression.
    """
    
    def __init__(self, vlm):
        self.vlm = vlm
        self.state = SpeedrunnerAgentState()
        self.memory_manager = MemoryManager()
        
        self.memory_file = ".pokeagent_cache/memory_speedrunner.json"
        ...
        logger.info("Initialized SpeedrunnerAgent with memory management")
    ...
    def get_location_memory_context(self, map_name: str) -> str:
        ...
        if current_memory:
            ...
            lines.append(f"📍 CURRENT LOCATION ({map_name}): ⚡ New area! Find the fastest route through!")
        
        all_memories = self.memory_manager.get_all_location_memories()
        other_memories = {k: v for k, v in all_memories.items() if k != map_name}
        
        if other_memories:
            lines.append(f"\n🗺️ ROUTE KNOWLEDGE ({len(other_memories)} locations):")
        ...
        return "\n".join(lines) if lines else "No route data yet - let's move fast!"
    ...
    def step(self, game_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if frame is None:
            logger.error("🚫 No frame in game_state for SpeedrunnerAgent.step")
            return {"action": "WAIT", "reasoning": "No frame available"}
        
        action = self.process_step(frame, game_state)
        return {"action": action, "reasoning": "Speedrunner agent decision"}
    
    def process_step(self, frame, game_state: Dict[str, Any]) -> str:
        """Main processing step for speedrunner mode."""
        if frame is None:
            logger.error("🚫 CRITICAL: SpeedrunnerAgent.process_step called with None frame")
            return "WAIT"
        ...
        try:
            ...
            # Speedrunner-specific prompt
            prompt = f"""You are a SPEEDRUNNER playing Pokemon Emerald. Your mission: PROGRESS FAST AND EFFICIENTLY!

🎯 SPEEDRUNNER OBJECTIVES:
- Focus on main story progression - get to the next gym/objective quickly
- Avoid unnecessary battles, exploration, or item collection
- Take the most direct route to your next goal
- Skip optional content unless absolutely necessary
- Move with purpose - no wandering or backtracking
...
Instructions:
1. Identify the next story objective
2. Take the fastest route to complete it
3. Skip optional battles and exploration

Response format:
ANALYSIS:
[Where am I? What's the next objective? Fastest route?]

LOCATION_MEMORY: [Record optimal routes, story checkpoints, required actions for speedrun efficiency]

PLAN:
[What's the fastest way forward? Any obstacles?]

ACTION:
[Your chosen action - prefer efficient movement toward goals]

Map: {map_name} | Time: {self.state.step_counter} steps | Step: {self.state.step_counter}"""
            
            print("\n" + "="*100)
            print("⚡ SPEEDRUNNER AGENT PROMPT:")
            print("="*100)
            ...
            response = self.vlm.get_query(frame, prompt, "speedrunner_mode", game_state=game_state)
            ...
            logger.error(f"Error in speedrunner agent processing: {e}")
            return "A"
...
_global_speedrunner_agent = None


def get_speedrunner_agent(vlm):
    """Get or create the global speedrunner agent instance"""
    global _global_speedrunner_agent
    if _global_speedrunner_agent is None:
        _global_speedrunner_agent = SpeedrunnerAgent(vlm)
    elif _global_speedrunner_agent.vlm != vlm:
        _global_speedrunner_agent = SpeedrunnerAgent(vlm)
    
    return _global_speedrunner_agent
