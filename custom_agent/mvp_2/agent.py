...
from custom_agent.mvp_2.modules.perception import PerceptionModule, PerceptionResult
from custom_agent.mvp_2.modules.saliency import SaliencyDetector
from custom_agent.mvp_2.modules.memory import EpisodicMemory
from custom_agent.mvp_2.modules.planner import Planner
from custom_agent.mvp_2.executors.navigation_executor import NavigationExecutor
from custom_agent.mvp_2.executors.battle_executor import BattleExecutor
from custom_agent.mvp_2.executors.dialogue_executor import DialogueExecutor
from custom_agent.mvp_2.executors.general_executor import GeneralExecutor
from custom_agent.mvp_2.executors.overall_executor_nt import OverallExecutorNT

from custom_agent.mvp_2.executors.base_executor import ExecutorResult, BaseExecutor
...
@AgentRegistry.register("mvp_two")
class MVPTwoAgent(BaseLangChainAgent):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        # NT: Track if we've reached LITTLEROOT TOWN (handover point)
        self.reached_littleroot_town = False
    ...
    def step(self, game_state: dict) -> dict:
        ...
        # NT: Startup override
        # Check if we've reached LITTLEROOT TOWN (handover point)
        player_data = game_state.get('player', {})
        player_map = player_data.get('location', 'unknown')
        # if player_map.upper() == "LITTLEROOT TOWN BRENDANS HOUSE 2F" and not self.reached_littleroot_town:
        #     logger.info("Player reached LITTLEROOT TOWN BRENDANS HOUSE 2F - switching from OverallExecutorNT to standard executors")
        #     self.reached_littleroot_town = True
        #     # Clear current executor to force replanning with standard executors
        #     self.agent_state.current_executor = None
        #     self.agent_state.current_executor_type = None
        #     self.agent_state.current_goal = None
        
        # # Use OverallExecutorNT exclusively until LITTLEROOT TOWN is reached
        # if not self.reached_littleroot_town:
        #     # Force OverallExecutorNT as current executor if not already set
        #     if self.agent_state.current_executor_type != 'overall':
        #         self.agent_state.current_executor_type = 'overall'
        #         self.agent_state.current_executor = self.executors['overall']
        #         self.agent_state.current_goal = "Navigate early game objectives until LITTLEROOT TOWN"
        #         logger.info("Using OverallExecutorNT for early game (before LITTLEROOT TOWN)")
            
        #     # Execute OverallExecutorNT step
        ...
        # NT: Check for dialogue every step
        frame = np.array(game_state.get('frame', []))
        dialogue_detected = False
        if frame.size > 0:
            dialogue_detected = detect_dialogue(frame, threshold=0.3)

        # If dialogue detected and not already using dialogue executor, switch to it
        if dialogue_detected and self.agent_state.current_executor_type != 'dialogue':
            logger.info("Dialogue detected - switching to dialogue executor")
            if self.agent_state.current_executor_type == 'navigation':
                self._suspend_current_goal()
            self.agent_state.current_executor_type = 'dialogue'
            self.agent_state.current_executor = self.executors['dialogue']
            self.agent_state.current_goal = "Handle dialogue advancement"
