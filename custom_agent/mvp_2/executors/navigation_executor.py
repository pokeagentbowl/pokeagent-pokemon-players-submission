...
from custom_utils.map_extractor_nt import extract_portal_connections
...
class NavigationExecutor(BaseExecutor):
    ...
    def execute_step(
        ...
        self.current_frame = getattr(perception, 'frame', None)
    ...
    def _update_state(self, state_data: dict) -> None:
        ...
        if self.player_map != self.last_map:
    ...
    def _execute_navigation(
        ...
        if self._has_reached_target(state_data):
            ...
            # Mark target tile as interacting before attempting interaction
            current_target = self.internal_state['current_target']
            if current_target and self.current_frame is not None:
                target_pos = current_target.map_tile_position
                mark_and_save_tile(
                    target_pos, 
                    self.current_frame, 
                    'interacting', 
                    self.active_tile_index, 
                    self.player_map,
                    allow_upgrade=True, 
                    player_map_tile_x=self.player_map_tile_x,
                    player_map_tile_y=self.player_map_tile_y
                )
                logger.info(f"Marked target tile {target_pos} as interacting")
        ...
        if self.internal_state['steps_since_progress'] >= self.stuck_threshold:
            ...
            # Mark current position as untraversable
            if self.current_frame is not None and self.player_map_tile_x is not None and self.player_map_tile_y is not None:
                mark_and_save_tile(
                    (self.player_map_tile_x, self.player_map_tile_y), 
                    self.current_frame, 
                    'untraversable', 
                    self.active_tile_index, 
                    self.player_map,
                ...
                logger.info(f"Marked stuck tile ({self.player_map_tile_x}, {self.player_map_tile_y}) as untraversable")
