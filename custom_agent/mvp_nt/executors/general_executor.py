"""General executor - handles menus, dialogues, and general interactions.
...
2. If continuing, what button(s) should you press next?
...
class GeneralExecutor(BaseExecutor):
    """
    General executor for menus, dialogue, and interactions.
    ...
    def __init__(self, reasoner: 'LangChainVLM'):
        """
        Initialize general executor with VLM reasoner.
    ...
    def is_still_valid(
        ...
        For MVP: always valid (VLM knows when to give up control).
        The VLM will set status='failed' if interrupted by battle, etc.
        ...
        Returns:
            bool: True (always valid for MVP)
        """
        # MVP: Always valid. VLM decides when to give up control via status field.
        return True
    ...
    def _decide_action_with_vlm(
        ...
        """
        Decide next button press using VLM with structured output.
        ...
        if is_dialog:
            has_dialog = "There is an active dialog box at the bottom of the screen."
        else:
            has_dialog = "There is no dialog box currently visible."
        # Format action history for prompt
        history_context = self._format_action_history()

        prompt = GENERAL_EXECUTOR_PROMPT.format(
            goal=goal,
