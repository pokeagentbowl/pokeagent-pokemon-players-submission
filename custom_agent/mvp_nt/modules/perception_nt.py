...
class PerceptionResult(BaseModel):
    ...
    frame: Optional[np.ndarray] = None  # Original frame, None if bypassed
...
class PerceptionModule:
    ...
    def process(self, game_state: dict) -> PerceptionResult:
        ...
        if USEPERCEPTIONOVERRIDE and (in_dialogue or not player_visible):
            ...
            # 2. Generate scene description (placeholder for MVP)
            scene_description = ""
        ...
        return PerceptionResult(
            ...
            frame=frame if not (USEPERCEPTIONOVERRIDE and (in_dialogue or not player_visible)) else None
        )
