"""
Navigation target generation from detected objects and traversability boundaries.
"""
import logging
...
from custom_utils.azure_vision_analyzer import DetectedObject
from custom_utils.coordinate_transform import (
    FRAME_HEIGHT,
    FRAME_WIDTH,
    LOCAL_TILE_HEIGHT_VISIBLE,
    LOCAL_TILE_WIDTH,
    local_tile_to_map_tile,
    pixel_to_local_tile,
)
...
class NavigationTarget(BaseModel):
    ...
    detected_object: DetectedObject | dict | None = None
    source_map_location: str | None = None
    tile_size: tuple[int, int] | None = None
    reachable: bool | None = None
    exit_direction: tuple[int, int] | None = None  # For doors: (dx, dy) indicating exit direction
...
def generate_navigation_targets(
    ...
    player_map_location: str | None = None
    ...
    for i, obj in enumerate(detected_objects):
        ...
        target = NavigationTarget(
            ...
            detected_object=obj.model_dump(),
    ...
    # Find and add known object targets (PC, TV, Bookshelf, Clock, NPC)
    known_object_positions = find_known_objects(traversability_map)

    # Map tile characters to descriptive names
    object_type_map = {
        'C': 'PC/Computer',
        'T': 'Television',
        'B': 'Bookshelf',
    ...
    for m, (local_tile_x, local_tile_y, tile_char) in enumerate(known_object_positions):
        # Convert local tile to map tile position
        map_tile_x, map_tile_y = local_tile_to_map_tile(local_tile_x, local_tile_y, player_map_tile_pos[0], player_map_tile_pos[1])

        # Get descriptive name for this object type
        object_name = object_type_map.get(tile_char, 'Unknown Object')

        # Create navigation target for known object
        target = NavigationTarget(
            id=f"object_{tile_char.lower()}_{m}",
            type="object",
            map_tile_position=(map_tile_x, map_tile_y),
            local_tile_position=(local_tile_x, local_tile_y),
            description=f"{object_name} at ({map_tile_x}, {map_tile_y}) of size 1x1",
            priority=0.0,
...
def find_known_objects(
    traversability_map: list[list[str]]
) -> list[tuple[int, int, str]]:
    """
    Find known objects (PC, TV, Bookshelf, Clock, NPC) in the traversability map.
    ...
    Returns:
        List of tuples (x, y, tile_char) where tile_char is one of C, T, B, K, N
    """
    object_positions = []
    ...
    for y in range(min_y, max_y + 1):
        for x in range(15):
            tile = traversability_map[y][x]
            if tile in {'C', 'T', 'B', 'K', 'N'}:
                object_positions.append((x, y, tile))
