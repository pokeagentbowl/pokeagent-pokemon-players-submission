...
def _coerce_to_bool(value: Any, default: bool = False) -> bool:
    """Best-effort conversion of arbitrary inputs to boolean flags."""
    if value is None:
        return default
...
class EmeraldEmulator:
    ...
    def __init__(self, rom_path: str, headless: bool = True, sound: bool = False):
        ...
        # Control whether state loads overwrite stitched map cache
        self.preserve_map_cache = _get_bool_env("AGENT_PRESERVE_MAP_CACHE", False)
    ...
    def load_state(
        ...
        preserve_map_cache: Optional[bool] = None
    ):
        ...
        try:
            effective_preserve = (
                self.preserve_map_cache if preserve_map_cache is None
                else _coerce_to_bool(preserve_map_cache, default=self.preserve_map_cache)
            )
            if preserve_map_cache is not None and effective_preserve != self.preserve_map_cache:
                logger.info(
                    f"Map cache preservation override requested: {effective_preserve} "
                    f"(default: {self.preserve_map_cache})"
                )
            if path:
            ...
            # print( Loading state from path: {path}")
                    ...
                    self._copy_state_files_to_cache(path, preserve_map_cache=effective_preserve)
    ...
    def _copy_state_files_to_cache(self, state_filename: str, preserve_map_cache: bool):
        ...
        if preserve_map_cache:
            if os.path.exists(cache_map_stitcher_file):
                logger.info("Preserving existing map stitcher cache; skipping state map copy")
            else:
                logger.info(
                    "Preserve map cache requested but no cache file found; creating empty cache file"
                )
                import json
