...
class PokemonEmeraldReader:
    ...
    def __init__(self, core, milestone_tracker=None):
        ...
        self._mem_cache = {}
        self._behavior_enum_cache: Dict[int, MetatileBehavior] = {}
    ...
    def _behavior_from_byte(self, behavior_byte: int) -> MetatileBehavior:
        """Convert cached behavior bytes to enums without repeated construction."""
        cached = self._behavior_enum_cache.get(behavior_byte)
        if cached is not None:
            return cached

        try:
            resolved = MetatileBehavior(behavior_byte)
        except ValueError:
            resolved = MetatileBehavior.NORMAL

        self._behavior_enum_cache[behavior_byte] = resolved
        return resolved
    ...
    def _read_bytes(self, address: int, length: int) -> bytes:
        ...
        try:
            ...
            if len(data) != length:
                logger.debug(
                    "Partial memory read at 0x%08X (expected %d, got %d)",
                    address,
                    length,
                    len(data),
                )
    ...
    def read_player_name(self) -> str:
        ...
        try:
            ...
            if decoded_name and len(decoded_name) >= 2:
                logger.debug("Read player name: '%s'", decoded_name)
                return decoded_name
    ...
    def read_party_size(self) -> int:
        ...
        try:
            ...
            logger.debug("Read party count: %d", party_count)
            return party_count
        except Exception as e:
    ...
    def read_party_pokemon(self) -> List[PokemonData]:
        ...
        try:
            ...
            logger.debug("Reading party with size: %d", party_size)
            ...
            for i in range(party_size):
                logger.debug("Reading Pokemon at slot %d", i)
                try:
                    ...
                    logger.debug("Slot %d parsed: %s", i, pokemon)
                except Exception as e:
    ...
    def read_map_around_player(
        self,
        radius: int = 7,
        *,
        position: Optional[Tuple[int, int]] = None,
        location: Optional[str] = None,
    ) -> List[List[Tuple[int, MetatileBehavior, int, int]]]:
        ...
        map_data = self._read_map_data_internal(radius, position)
        ...
        if map_data and len(map_data) > 0:
            ...
            if is_outdoor:
                ...
                if unknown_ratio > 0.5:
                    logger.debug(
                        "Outdoor map has %.1f%% unknown tiles, retrying with cache invalidation",
                        unknown_ratio * 100,
                    )
                    self.invalidate_map_cache()
                    if self._find_map_buffer_addresses():
        ...
        if map_data:
            rows = len(map_data)
            cols = len(map_data[0]) if map_data[0] else 0
            logger.debug("Map data read: %dx%d region", rows, cols)
    ...
    def read_map_metatiles(self, x_start: int = 0, y_start: int = 0, width: int = None, height: int = None) -> List[List[Tuple[int, MetatileBehavior, int, int]]]:
        ...
        try:
            buffer_start = self._map_buffer_addr
            buffer_size = self._map_width * self._map_height * 2
            buffer_end = buffer_start + buffer_size
            behaviors = self.get_all_metatile_behaviors()
            metatiles: List[List[Tuple[int, MetatileBehavior, int, int]]] = []
            row_byte_count = width * 2

            for y in range(y_start, y_start + height):
                row: List[Tuple[int, MetatileBehavior, int, int]] = []
                row_index = ((y * self._map_width) + x_start) * 2
                row_addr = buffer_start + row_index

                if row_addr < buffer_start or row_addr + row_byte_count > buffer_end:
                    logger.debug("Invalid metatile row address: 0x%08X", row_addr)
                    row.extend([(0, MetatileBehavior.NORMAL, 0, 0)] * width)
                    metatiles.append(row)
                    continue

                row_bytes = self.read_memory(row_addr, row_byte_count)
                if len(row_bytes) != row_byte_count:
                    logger.debug(
                        "Short metatile row read at 0x%08X (expected %d, got %d)",
                        row_addr,
                        row_byte_count,
                        len(row_bytes),
                    )
                ...
                for offset in range(0, row_byte_count, 2):
                    metatile_value = row_bytes[offset] | (row_bytes[offset + 1] << 8)
                    ...
                    if metatile_id > 0x3FF:
                        logger.debug("Invalid metatile ID: %d", metatile_id)
                        metatile_id = 0

                    if behaviors and metatile_id < len(behaviors):
                        behavior_byte = behaviors[metatile_id]
                        behavior = self._behavior_from_byte(behavior_byte)
                    else:
    ...
    def get_comprehensive_state(self, screenshot=None) -> Dict[str, Any]:
        ...
        logger.debug("Starting comprehensive state reading")
        state = {
        ...
        try:
            ...
            state = self.read_map(state, player_coords=coords, location=location)
            ...
            if self._dialog_detection_enabled:
                ...
                if dialog_text:
                    ...
                    logger.debug("Dialog text detected (%.0f chars)", len(dialog_text))
                else:
            ...
            # Party Pokemon
            party = self.read_party_pokemon()
            logger.debug("Party size detected: %d", len(party) if party else 0)
            if party:
                state["player"]["party"] = [
                    {
    ...
    def read_map(
        ...
        *,
        ...
        coords = player_coords or self.read_coordinates()
        tiles = self.read_map_around_player(radius=7, position=coords, location=location)
        if tiles:
            ...
            unknown_ratio = unknown_count / total_tiles if total_tiles > 0 else 0
            logger.debug(
                "Tile sample: %.1f%% unknown (%d/%d), %d corrupted",
                unknown_ratio * 100,
                unknown_count,
                total_tiles,
                corruption_count,
        ...
        if object_events:
            ...
            logger.debug("Found %d object events in current map", len(object_events))
            
            # Add player absolute coordinates for NPC positioning
            if coords:
                state["map"]["player_coords"] = {'x': coords[0], 'y': coords[1]}
        else:
        ...
        if self._map_stitcher and state.get("player"):
            player_location = state["player"].get("location", "Unknown")
            player_position = state["player"].get("position")
            player_pos = (
            ...
            if (
                player_location
                and player_location not in [None, "None", "Unknown"]
                and player_position
            ):
                # Get connections with coordinates for this location
                connections_with_coords = []
                if player_location and self._map_stitcher:
                    try:
                        location_connections = self._map_stitcher.get_location_connections(player_location)
    ...
    def read_dialog_with_ocr_fallback(self, screenshot=None) -> str:
        ...
        # OPTIMIZATION: Skip expensive OCR if we're not even in a dialog state
        # This prevents 5+ second delays on every state read
    ...
    def read_object_events(self):
        ...
        try:
            ...
            logger.debug(
                "Found %d NPCs/trainers near player at (%d, %d)",
                len(filtered_events),
                player_x,
                player_y,
            )
