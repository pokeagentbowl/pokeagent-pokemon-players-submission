...
def _coerce_request_bool(value, default=None):
    """Convert incoming JSON values to booleans safely."""
    if value is None:
        return default
...
current_obs = None
current_frame_png = None
current_frame_jpeg = None
current_frame_resolution = (0, 0)
fps = 80
...
video_frames_written = 0  # Track actual frames written to file
video_frame_skip = 4  # Record every 4th frame (80/4 = 20 FPS)
...
def init_video_recording(record_enabled=False):
    ...
    global video_writer, video_recording, video_filename, fps, video_frame_skip, video_frames_written, video_frame_counter
    ...
    try:
        ...
        # Create timestamp and resolve session ID
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        
        # Resolve LANGFUSE_SESSION_ID from template
        session_id = os.environ.get("LANGFUSE_SESSION_ID", "")
        if "{" in session_id:
            # Replace template variables
            session_id = session_id.replace("{DATETIME}", timestamp)
            session_id = session_id.replace("{AGENT_MODE}", os.environ.get("AGENT_MODE", "unknown"))
            session_id = session_id.replace("{AGENT_TYPE}", os.environ.get("AGENT_TYPE", "unknown"))
            session_id = session_id.replace("{AGENT_MODEL_NAME}", os.environ.get("AGENT_MODEL_NAME", "unknown"))
            session_id = session_id.replace("{AGENT_BACKEND}", os.environ.get("AGENT_BACKEND", "unknown"))
            # Backward compatibility
            agent_mode = os.environ.get("AGENT_MODE", "unknown")
            agent_type = os.environ.get("AGENT_TYPE", "unknown")
            session_id = session_id.replace("{AGENT_SCAFFOLD}", f"{agent_mode}_{agent_type}")
        
        # Determine recording directory
        submission_dir = os.environ.get("SUBMISSION_DIR", "submissions")
        if session_id:
            recording_dir = f"{submission_dir}/{session_id}"
        else:
        ...
        # Record at target FPS (skip frames from emulator FPS)
        recording_fps = fps / video_frame_skip
        
        # Try multiple codec/container combinations in order of preference  
        # Using AVI for better resilience (writes frames immediately without needing finalization)
        # MP4 requires proper finalization and can be corrupted if process is killed
        codecs_to_try = [
            ('XVID', 'avi', 'XVID in AVI'),       # Best: good compression, resilient to interruption
            ('mp4v', 'avi', 'MPEG-4 in AVI'),     # Good compression in AVI
            ('FMP4', 'avi', 'FFmpeg MPEG-4'),     # FFmpeg MPEG-4 in AVI
            ('DIVX', 'avi', 'DivX in AVI'),       # DivX in AVI
            ('MJPG', 'avi', 'Motion JPEG'),       # Last resort: large files but most reliable
        ]
        
        video_writer = None
        for codec_str, ext, name in codecs_to_try:
            video_filename = f"{recording_dir}/pokegent_recording_{timestamp}.{ext}"
            try:
                fourcc = cv2.VideoWriter_fourcc(*codec_str)
                test_writer = cv2.VideoWriter(video_filename, fourcc, float(recording_fps), (240, 160))
                
                if test_writer.isOpened():
                    video_writer = test_writer
                    print(f"✅ Using {name} codec for recording")
                    break
                else:
                    test_writer.release()
            except Exception as e:
                print(f"⚠️  {name} codec failed: {e}")
                continue
        
        if video_writer and video_writer.isOpened():
            video_recording = True
            print(f"📹 Video recording started: {video_filename} at {recording_fps:.1f} FPS (recording every {video_frame_skip} frames)")
        else:
            print("❌ Failed to initialize video recording - no working codec found")
            print("   This might be due to missing ffmpeg or codec libraries in the container")
            if video_writer:
...
def record_frame(screenshot):
    ...
    global video_writer, video_recording, video_frame_counter, video_frame_skip, video_frames_written
    ...
    try:
        ...
        if hasattr(screenshot, 'save'):  # PIL image
            ...
            if video_frames_written % 100 == 0:
                import logging
                logger = logging.getLogger(__name__)
                logger.info(f"📹 Recorded {video_frames_written} frames ({video_frames_written / 20:.1f}s of video)")
...
def cleanup_video_recording():
    ...
    global video_writer, video_recording, video_filename, video_frames_written
    ...
    if video_recording and video_writer is not None:
        try:
            ...
            duration_seconds = video_frames_written / 20.0  # 20 FPS
            
            # Verify the file was created properly
            if os.path.exists(video_filename):
                file_size = os.path.getsize(video_filename)
                print(f"📹 Video recording saved: {video_filename}")
                print(f"   Size: {file_size:,} bytes | Frames: {video_frames_written} | Duration: {duration_seconds:.1f}s")
                
                # Try to verify the video is valid using ffmpeg
                try:
                    import subprocess
                    result = subprocess.run(
                        ['ffprobe', '-v', 'error', '-show_format', video_filename],
                        capture_output=True,
                        timeout=5
                    )
                    if result.returncode == 0:
                        ...
                        print(f"⚠️  Video file may have issues (ffprobe failed)")
                except Exception:
                    pass  # ffprobe not available or failed, skip validation
            else:
                print(f"⚠️  Video file was not created: {video_filename}")
...
async def lifespan(app: FastAPI):
    # Startup
    yield
    # Shutdown - ensure video is properly saved
    print("\n🛑 Server shutting down, finalizing video recording...")
...
def step_environment(actions_pressed):
    ...
    global current_obs, current_frame_png, current_frame_jpeg, current_frame_resolution
    ...
    try:
        ...
        if screenshot:
            ...
            try:
                ...
                success, jpeg_buffer = cv2.imencode(".jpg", bgr_image, [int(cv2.IMWRITE_JPEG_QUALITY), 85])
                if success:
                    jpeg_bytes = jpeg_buffer.tobytes()
            except Exception as e:
                logger.debug(f"JPEG encode failed: {e}")
            with obs_lock:
                current_obs = np_image
                current_frame_png = png_bytes
                if jpeg_bytes:
                    current_frame_jpeg = jpeg_bytes
                current_frame_resolution = (np_image.shape[1], np_image.shape[0])
...
@app.get("/stream_interactive")
async def get_stream_interactive():
    """Serve the interactive stream interface (screenshot + map + buttons)"""
    try:
        with open("server/stream_interactive.html", "r") as f:
            content = f.read()
        return HTMLResponse(content=content)
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail="Interactive stream interface not found")
...
async def get_screenshot():
    ...
    global current_frame_png, current_frame_resolution

    if env is None:
        raise HTTPException(status_code=400, detail="Emulator not initialized")

    with obs_lock:
        frame_bytes = current_frame_png
        frame_resolution = current_frame_resolution
    ...
    if width and height:
        headers["X-Frame-Width"] = str(width)
        headers["X-Frame-Height"] = str(height)

    with step_lock:
        headers["X-Step-Number"] = str(step_count)

    return Response(content=frame_bytes, media_type="image/png", headers=headers)
...
async def frame_stream():
    """Stream frames as MJPEG for efficient playback."""
    ...
    try:
        ...
        img_str = base64.b64encode(frame_bytes).decode()
...
async def get_agent_config():
    """Get agent configuration information (scaffold and port)"""
    import os
    
    # Get configuration from environment variables
    agent_scaffold = os.getenv("AGENT_MODE", "unknown") + ", " + os.getenv("AGENT_TYPE", "") + ", " + os.getenv("AGENT_MODEL_NAME", "")
...
async def save_state_endpoint(request: SaveStateRequest):
    """
    Save the current emulator state to a folder.
    
    Only folder-based saves are supported.
    Automatically includes agent memory from .pokeagent_cache/memory.json if available from cache.
    
    Request format:
    {
        "folder_path": "path/to/folder"   # Required: folder for saves
    }
    ...
    try:
        ...
        # Only folder_path is allowed
        if not folder_path:
            return JSONResponse(status_code=400, content={"error": "Must provide 'folder_path'. Single file saves are no longer supported."})
        
        # Folder-based save mode
        os.makedirs(folder_path, exist_ok=True)
        state_file = os.path.join(folder_path, folder_path + ".state")
        
        if env:
            # Save emulator state
            env.save_state(state_file)
            logger.info(f"💾 Saved emulator state to: {state_file}")
            
            # Save milestones
            if env.milestone_tracker:
                milestone_file = os.path.join(folder_path, "milestones.json")
                milestone_data = {
                    "milestones": env.milestone_tracker.milestones,
                    "timestamp": datetime.datetime.now().isoformat()
                }
                with open(milestone_file, "w") as f:
                    json.dump(milestone_data, f, indent=2)
                logger.info(f"💾 Saved milestones to: {milestone_file}")
            
            # Auto-save agent memory from cache (like map stitching data)
            # Agent auto-saves to .pokeagent_cache/memory.json, we copy it here
            cache_memory_file = ".pokeagent_cache/memory.json"
            if os.path.exists(cache_memory_file):
                import shutil
                memory_file = os.path.join(folder_path, "memory.json")
                shutil.copy2(cache_memory_file, memory_file)
                logger.info(f"💾 Saved agent memory to: {memory_file}")
            
            return {
                "status": "success",
                "message": f"State saved to folder {folder_path}",
                "files": {
                    "state": state_file,
                    "milestones": os.path.join(folder_path, "milestones.json") if env.milestone_tracker else None,
                    "memory": os.path.join(folder_path, "memory.json") if os.path.exists(cache_memory_file) else None
                }
            }
        else:
...
async def load_state_endpoint(request: LoadStateRequest):
    """
    Load an emulator state from a folder.
    
    Only folder-based loads are supported.
    
    Request format:
    {
        "folder_path": "path/to/folder",   # Required: folder with saves
        "preserve_map_cache": true (default) or false  # Optional: override to not preserve map cache on load
    }
    
    Response includes memory.json data if available for client to load.
    """
    try:
        folder_path = request.folder_path
        preserve_override = _coerce_request_bool(request.preserve_map_cache)
        ...
        # Only folder_path is allowed
        if not folder_path:
            return JSONResponse(status_code=400, content={"error": "Must provide 'folder_path'. Single file loads are no longer supported."})
        
        # Folder-based load mode
        state_file = os.path.join(folder_path, f"{os.path.basename(folder_path)}.state")
        
        if not os.path.exists(state_file):
            return JSONResponse(status_code=404, content={"error": f"State file not found: {state_file}"})
        
        if env:
            # Load emulator state
            load_kwargs = {}
            if preserve_override is not None:
                load_kwargs["preserve_map_cache"] = preserve_override
            env.load_state(state_file, **load_kwargs)
            logger.info(f"📂 Loaded emulator state from: {state_file}")
            ...
            if os.path.exists(milestone_file):
                ...
                if env.milestone_tracker and "milestones" in milestone_data:
                    env.milestone_tracker.milestones = milestone_data["milestones"]
                logger.info(f"📂 Loaded milestones from: {milestone_file}")
            
            # Load memory.json if available (return to client)
            memory_file = os.path.join(folder_path, "memory.json")
            if os.path.exists(memory_file):
                with open(memory_file, "r") as f:
                    memory_data = json.load(f)
                logger.info(f"📂 Found agent memory at: {memory_file}")
            
            return {
                "status": "success",
                "message": f"State loaded from folder {folder_path}",
                "memory_data": memory_data,  # Client can use this to restore agent memory
                "files_loaded": {
                    "state": state_file,
                    "milestones": milestone_file if os.path.exists(milestone_file) else None,
...
@app.post("/load_state_classic")
async def load_state_endpoint_classic(request: dict):
    """
    Load an emulator state from a file as per previous. 
    previous one was overriden
    """
    try:
...
def main():
    ...
    # Check for recording environment variables (support both RECORD_VIDEO and AGENT_RECORD)
    if not args.record:
        env_record = os.environ.get("RECORD_VIDEO") or os.environ.get("AGENT_RECORD") or ""
        env_record_lower = env_record.lower()
        if env_record_lower in ("true", "1", "yes", "on"):
            args.record = True
            print(f"🎥 Video recording enabled from environment variable")
...
def init_for_multiprocess():
    ...
    if env is None:  # Only initialize once
        ...
        # Support both RECORD_VIDEO and AGENT_RECORD, handle true/1/yes/on
        env_record = os.environ.get("RECORD_VIDEO") or os.environ.get("AGENT_RECORD") or ""
        record_video = env_record.lower() in ("true", "1", "yes", "on")
