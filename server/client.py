...
def run_multiprocess_client(server_port=8000, args=None, server_host="localhost"):
    """
    ...
    Args:
        ...
        server_host: Host where the server is running (default: localhost)
    ...
    while running:
        try:
                    ...
                    if event.type == pygame.QUIT:
                        ...
                        # Manual controls (only in manual mode)
                            ...
                            if event.key in (pygame.K_UP, pygame.K_w):
                                ...
                                # Save state (folder-based)
                                print("💾 Saving state (agent memory auto-saved)...")
                                try:
                                    # Simple save request - memory auto-saved to .pokeagent_cache/memory.json
                                    save_request = {
                                    ...
                                    if response.status_code == 200:
                                        ...
                                        print(f"✅ State saved to {save_request['folder_path']}")
                                        if result.get("files"):
                                            files = result["files"]
                                            if files.get("memory"):
                                                print(f"   ✅ Agent memory included")
                                    else:
                                        print(f"❌ Failed to save state: {response.status_code}")
                                except Exception as e:
                                    print(f"❌ Error saving state: {e}")
                            elif event.key == pygame.K_2:
                                # Load state (folder-based)
                                print("📂 Loading state (with agent memory if available)...")
                                try:
                                    ...
                                    if response.status_code == 200:
                                        result = response.json()
                                        print(f"✅ State loaded from .pokeagent_cache/manual_save")
                                        
                                        # Restore agent memory if available
                                        memory_data = result.get("memory_data")
                                        if memory_data and hasattr(agent, 'agent_impl') and hasattr(agent.agent_impl, 'memory_manager'):
                                            agent.agent_impl.memory_manager.from_dict(memory_data)
                                            print(f"   ✅ Agent memory restored")
                                        elif memory_data:
                                            print(f"   ⚠️ Memory data found but agent doesn't support memory")
                                    else:
            ...
            if mode == "AUTO":
                # Check if action queue is ready (no time delay, immediate processing)
                try:
                    queue_response = requests.get(f"{server_url}/queue_status", timeout=2)
                                    ...
                                    if result and result.get('action'):
                                        ...
                                        try:
                                            ...
                                            if response.status_code == 200:
                                                ...
                                                # Apply configurable action delay from environment
                                                action_delay = float(os.getenv('AGENT_ACTION_DELAY', '1.0'))
                                                ...
                                                try:
                                                        ...
                                                        if client_llm_logger:
                                                            ...
                                                            if sync_response.status_code == 200:
                                                                if step_count % 10 == 0:  # Log every 10 steps to avoid spam
                                                                    logger.debug(f"🔄 LLM metrics synced to server")
                                                    except Exception as e:
                                                        logger.warning(f"⚠️ LLM metrics sync error: {e}")
                                                    ...
                                                    if checkpoint_response.status_code == 200 and history_response.status_code == 200:
                                                        if step_count % 10 == 0:  # Log every 10 steps to avoid spam
                                                            logger.debug(f"💾 Checkpoint and history saved at step {step_count}")
                                                    else:
                                                        logger.warning(f"⚠️ Save failed - Checkpoint: {checkpoint_response.status_code}, History: {history_response.status_code}")
                                                except requests.exceptions.RequestException as e:
                                                    logger.warning(f"⚠️ Checkpoint/history save error: {e}")
                                            else:
                                                print(f"🎮 Agent: {action} (server error: {response.status_code})")
                                        except requests.exceptions.RequestException as e:
                                            print(f"🎮 Agent: {action} (connection error: {e})")
                except Exception as e:
                    logger.exception(f"❌ AUTO mode error: {e}")            # Small sleep to prevent CPU spinning
            if headless:
                time.sleep(0.1)
