...
class AntiCheatTracker:
    ...
    def __init__(self, submission_log_path=None):
        ...
        # Determine submission log path
        if submission_log_path is None:
            submission_log_path = self._get_default_submission_log_path()
        self.submission_log_path = submission_log_path
        
        # Ensure directory exists
        os.makedirs(os.path.dirname(self.submission_log_path), exist_ok=True)
        ...
        if not self.submission_logger.handlers:
            submission_handler = logging.FileHandler(self.submission_log_path, mode='a')
    ...
    def _get_default_submission_log_path(self):
        """Determine the default submission log path based on environment variables"""
        ...
        if session_id:
            return f"{submission_dir}/{session_id}/submission.log"
        else:
