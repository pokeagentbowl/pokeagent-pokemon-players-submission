...
def save_checkpoint(emulator, llm_logger=None, agent_step_count=0, memory_agent=None):
    """
    Save a complete checkpoint including game state, milestones, LLM history, and agent memory.
    ...
    Args:
        ...
        memory_agent: Optional agent instance with memory_manager attribute for saving memory
                     (e.g., LocationSimpleAgent or any agent with memory_manager)
    ...
    try:
        ...
        # Save agent memory if memory_agent is provided and has memory_manager
        if memory_agent:
            try:
                os.makedirs(".pokeagent_cache", exist_ok=True)
                memory_file = ".pokeagent_cache/memory.json"
                
                # Check if agent has memory_manager attribute
                if hasattr(memory_agent, 'memory_manager'):
                    # Use memory_manager.save_to_file directly
                    if memory_agent.memory_manager.save_to_file(memory_file):
                        print(f"   ✅ Saved agent memory to {memory_file}")
                    else:
                        print(f"   ⚠️ Failed to save agent memory")
                elif hasattr(memory_agent, 'save_memory'):
                    # Fallback to save_memory method for backward compatibility
                    if memory_agent.save_memory(memory_file):
                        print(f"   ✅ Saved agent memory to {memory_file}")
                    else:
                        print(f"   ⚠️ Failed to save agent memory")
                else:
                    print(f"   ℹ️ Agent doesn't have memory_manager or save_memory method")
            except Exception as e:
...
def load_checkpoint(emulator, llm_logger=None, memory_agent=None):
    """
    Load a checkpoint including game state, milestones, LLM history, and agent memory.
    
    Args:
        emulator: The game emulator instance
        llm_logger: Optional LLM logger for loading conversation history
        memory_agent: Optional agent instance with memory_manager attribute for loading memory
                     (e.g., LocationSimpleAgent or any agent with memory_manager)
    ...
    try:
        ...
        # Load agent memory if memory_agent is provided and has memory_manager
        if memory_agent:
            try:
                memory_file = ".pokeagent_cache/memory.json"
                if os.path.exists(memory_file):
                    # Check if agent has memory_manager attribute
                    if hasattr(memory_agent, 'memory_manager'):
                        # Use memory_manager.load_from_file directly
                        if memory_agent.memory_manager.load_from_file(memory_file):
                            print(f"   ✅ Loaded agent memory from {memory_file}")
                        else:
                            print(f"   ⚠️ Failed to load agent memory")
                    elif hasattr(memory_agent, 'load_memory'):
                        # Fallback to load_memory method for backward compatibility
                        if memory_agent.load_memory(memory_file):
                            print(f"   ✅ Loaded agent memory from {memory_file}")
                        else:
                            print(f"   ⚠️ Failed to load agent memory")
                    else:
                        print(f"   ℹ️ Agent doesn't have memory_manager or load_memory method")
                else:
                    print(f"   ℹ️ No agent memory file found at {memory_file}")
            except Exception as e:
